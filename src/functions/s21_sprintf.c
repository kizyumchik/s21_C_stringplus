...
int s21_sprintf(char *str, const char *format, ...) {
  ...
  while (*format) {
    ...
    format = handel_flags(format, &f);
    format = handel_width(format, &f, va);
    format = handel_precision(format, &f, va);
    format = handel__length(format, &f);
...
const char *handel_width(const char *format, flags *f, va_list args) {
  if (*format == '*') {
    format++;
    f->width = va_arg(args, int);
  } else if (s21_isdigit(*format)) {
    f->width = parse_number(&format);
  }
  return format;
}

const char *handel_precision(const char *format, flags *f, va_list args) {
  if (*format == '.') {
  ...
  if (*format == '*') {
    ...
    f->precision = parse_number(&format);
  }
  return format;
}
...
void handle_value(flags f, char *buff, va_list va) {
  if (f.specifier == 'd' || f.specifier == 'i')
    ...
    parse_and_format_mantissa(f, buff, va);
...
void parse_int(flags f, char *buff, va_list va) {
  ...
  full_num_to_string(val, buff, 10);
...
void parse_octal(flags f, char *buff, va_list va) {
  buff[0] = '0';
  full_num_to_string(va_arg(va, int64_t), buff + f.hash, 8);
  format_precision(buff, f);
  format_flags(buff, f);
}
...
void parse_char(flags f, char *buff, va_list args) {
  if (f.length == 'l') {
    wchar_t w_c;
